...
const ComplaintForm = ({ handleSubmit, userInput, setUserInput, isLoading }: ComplaintFormProps) => (
            ...
            <div className="space-y-3">
                {steps.map((step, index) => (
                    <div key={index} className="flex items-center text-base">
                        {step.status === 'pending' && <Spinner className="h-5 w-5 mr-3 text-gray-300"/>}
                        {step.status === 'loading' && <Spinner className="h-5 w-5 mr-3 text-blue-500"/>}
                        {step.status === 'complete' && <Check className="h-5 w-5 mr-3 text-green-500"/>}
                        <span className={
...
const SuggestedContacts = ({ contacts, rationale, isLoading, renderSocialHandle, generatedText }: SuggestedContactsProps) => (
    <Card className="shadow-md dark:bg-card">
        <CardHeader>
        ...
        <CardContent className="p-4">
                        ...
                        <div key={index} className="p-3 border rounded-lg dark:border-gray-700">
                            ...
                            {index === 0 && rationale && (
                                <p className="text-sm text-gray-600 dark:text-gray-400 mt-2 pt-2 border-t dark:border-gray-700">
                                    <span
                                        className="font-semibold text-gray-700 dark:text-gray-300">Why?</span> {rationale}
                                </p>
                            )}
                            ...
                            {index === 0 && (
                                <div className="pt-2 mt-2 border-t dark:border-gray-700">
                                    <Label
                                        className="text-xs font-semibold text-gray-500 dark:text-gray-400">Official
                                        X/Twitter</Label>
                                    <div className="mt-1">
                                        {isLoading ? (
            ...
            {!isLoading && !contacts.length && generatedText && (
                <p className="text-base text-gray-500 dark:text-gray-400">No specific contacts
                    found.</p>
            )}
        </CardContent>
    </Card>
...
export default function HomePage() {
    ...
    const [socialHandle, setSocialHandle] = useState<SocialHandleInfo | null>(null);
    ...
    const renderSocialHandle = () => {
        ...
        return (
            ...
            >
                ...
                <span
                    className="font-mono text-sm text-blue-700 dark:text-blue-400 font-medium">{socialHandle.handle}</span>
                {isVerified && (
                    <Badge variant="secondary"
            ...
            <div className="w-full max-w-3xl mx-auto">
                ...
                {(isLoading || generatedText) && (
                    <div className="w-full mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <GeneratedComplaint generatedText={generatedText} isLoading={isLoading} />
                    </div>
                )}
            </div>
        </main>
